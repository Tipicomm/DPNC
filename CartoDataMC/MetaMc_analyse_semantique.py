...
INPUT = "CartoDataMC/cartographie_ressources_datasets_proprietes.csv"
AXES_FILE = "/mnt/data/ModeleMetaMC_UTF8.csv"
...
BATCH_SIZE = 10
batches = [df_full[i:i + BATCH_SIZE] for i in range(ROW_START, ROW_END, BATCH_SIZE)]
...
- infos : score de confiance, nombre de valeurs distinctes, valeurs manquantes
- exemples : jusqu'à 3 valeurs typiques prélevées dans le fichier

Objectif :
Pour chaque propriété analysée, produire les colonnes suivantes :
- definition : une reformulation claire et concise du sens de la propriété, en s’appuyant sur les exemples si utiles
...
for idx, batch in enumerate(batches):
    ...
    for _, row in batch.iterrows():
        context = (
            ...
            f" | python_type: {row.get('python_type', '')}"
            f" | infos: score={row.get('score', '')}, distinct={row.get('nb_distinct', '')}, missing={row.get('nb_missing_values', '')}"
        )
        exemples = [row.get(col, '') for col in ['top_1', 'top_2', 'top_3'] if pd.notna(row.get(col, '')) and row.get(col, '').strip()]
        if exemples:
            context += f" | exemples: [{', '.join(exemples)}]"
